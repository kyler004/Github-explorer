...
import { formatData, formatNumber } from "../utils/format";
...
const RepoCard = ({
  ...
  const baseClasses = isEnhanced
    ? "bg-gray-800 rounded-xl p-6 hover:bg-gray-750 transition-all duration-200 border border-gray-700 hover:border-blue-500 hover:shadow-blue-500/20"
  ...
  return (
    <div className={baseClasses}>
      <div className="flex items-center justify-between mb-3">
        ...
        {repo.private && (
          <span
            className={`ml-2 px-2 py-1 text-xs bg-yellow-600 text-white rounded ${isEnhanced ? "shrink-0" : ""}`}
          ></span>
        )}
      </div>
      {repo.description && (
      ...
      <div className="flex flex-col gap-2">
        {repo.language && <div className="flex items-center text-gray-500"></div>}
        <div className={statsLayout}>
          ...
          <span className="flex items-center">
            ...
            {formatData(repo.updated_at, dateFormat)}
          </span>
        </div>
      </div>
    </div>
