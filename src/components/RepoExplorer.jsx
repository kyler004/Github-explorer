...
const RepoExplorer = () => {
    ...
    const handleSearch = async (error, pageNum = 1) => {
    ...
    const containerClass = `gb-linear-to-br from-gray-900 via-gray-900 py-8 px-4 ${hasSearched ? 'min-h-full' : 'h-full flex items-center justify-center'}`
  ...
  return (
    <div className={`max-w-7xl mx-auto ${hasSearched ? '' : 'w-full'}`}>
        ...
        { hasSearched && <RepoFilters sort={sort} order={order} onSortChange={(v) => handleFilterChange('sort', v)} onOrderChange={handleFilterChange(order, v)} disabled={loading} />}
        ...
        {repos.length > 0 && (
         ...
         {hasSearched && repos.length === 0 && !loading && !error && <div className="text-center py-12"><p className="text-gray-400 text-lg">No repositories found. Try a different search query</p></div>}     
    </div>
  )
}
